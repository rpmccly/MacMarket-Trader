...
vi.mock("@/components/operator-ui", async () => {
  ...
  return {
    ...
    StatusBadge: ({ tone, children }: { tone?: string; children: ReactNode }) =>
      ReactModule.createElement("span", { className: `op-badge op-badge-${tone ?? "neutral"}` }, children),
  };
});
...
describe("MomentumSummaryPanel", () => {
  ...
  it("renders a loading state with role=status when loading and payload is null", () => {
  ...
  it("renders score, trend, momo, components, parity, and HTF source for a bull payload (full mode)", () => {
  ...
  it("compact mode keeps the deterministic note and renders the score grid", () => {
    const html = renderToStaticMarkup(<MomentumSummaryPanel payload={payload()} compact />);
    expect(html).toContain("Bull +90");
    expect(html).toContain(DETERMINISTIC_NOTE);
    expect(html).toContain("op-grid-2"); // compact grid class
  });

  it("renders reversal and pullback warning badges prominently when explanation flags are set", () => {
    const data = payload();
    data.explanation = {
      ...data.explanation!,
    ...
    // Warnings are wrapped in <strong> for prominence.
    expect(html).toMatch(/<strong[^>]*data-testid="momentum-reversal-warning"/);
    expect(html).toMatch(/<strong[^>]*data-testid="momentum-pullback-signal"/);
  });

  it("renders bear payload with bad-tone badges and never frames neutral as bullish", () => {
    const data = payload({
    ...
    expect(html).toContain("op-badge-bad");
  });

  it("renders neutral payload with neutral badge tone (never bullish-tinted)", () => {
    const data = payload({
      latest_snapshot: snapshot({
        total_score: 10,
        total_label: "Neutral",
    ...
    const html = renderToStaticMarkup(<MomentumSummaryPanel payload={data} />);
    expect(html).toContain("Neutral +10");
    expect(html).toContain("State: neutral");
    // Header tone for neutral is "neutral", not "good".
    expect(html).toMatch(/op-badge-neutral[^"]*"[^>]*>Neutral \+10/);
  });
});
