...
[`thinkorswim-momentum-parity.md`](thinkorswim-momentum-parity.md).
The resolved workflow status — `missing`, `partial`, `ready`,
`passed`, `failed`, `pending` — is exposed on `MomentumRankingStatus`
as `thinkorswim_parity_workflow_status` and rendered in the
Settings card.
...
tests/fixtures/thinkorswim_momentum/
  ...
  parity-report.json           -- generated by the validator CLI (--write-report)
  parity-report.md             -- generated by the validator CLI (--write-report)

src/macmarket_trader/indicators/thinkorswim_parity.py
                                     -- production parsers, validation, comparison engine,
                                        report writer, and status builder
scripts/validate_thinkorswim_momentum_parity.py
                                     -- operator CLI (--write-report / --strict / --json)
tests/test_momentum_thinkorswim_parity_scaffold.py
                                     -- the legacy parity test (real-fixture-gated)
tests/test_momentum_parity_helper.py
                                     -- helper-level unit tests
tests/test_thinkorswim_momentum_parity_workflow.py
                                     -- workflow + CLI tests with synthetic fixtures
tests/helpers/momentum_parity.py     -- thin re-export shim for the legacy import path
```
